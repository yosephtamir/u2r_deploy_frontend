...
import { clearUserWishList } from '@/http/marketplace';
...
interface ClearWishListProps {
  closeModal: () => void;
}

const ClearYourWishList: React.FC<ClearWishListProps> = ({ closeModal }) => {
  const dispatch = useDispatch()
  const queryClient = useQueryClient();
  
  const { mutate: ClearWishList } = useAuthMutation(clearUserWishList(), {
		onSuccess: (data) => {
      onDeleteWishListItemSuccess(data),
      queryClient.invalidateQueries(['WishList-items']);
      queryClient.invalidateQueries(['user-WishList']);      
    },
		onError: (error) => onDeleteWishListItemError(error),
	});

  const onDeleteWishListItemSuccess = (data: any) => {
			console.log(data);
	...
	const onDeleteWishListItemError = (error: any) => {
		console.log(error);
		notify({
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-[#00000080] bg-opacity-30">
      <div id="modal" className="bg-white-100 p-4 rounded-lg w-[90%] md:w-[50%] lg:w-[24%] h-415 text-center ">
        ...
        <div>
          ...
          {/* Modal content */}
          <h2 className="text-[18px] font-bold pt-2 text-[#101828]">Clear WishList</h2>
          <p className="text-[#475467] text-[14px] font-normal mt-4 mb-2">
            Are you sure you want to remove all items from your Wish List?
          </p>
        </div>
        {/* Buttons */}
...
export default ClearYourWishList;
