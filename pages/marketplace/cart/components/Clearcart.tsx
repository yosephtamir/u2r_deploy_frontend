...
const ClearYourCart: React.FC<ClearCartProps> = ({ closeModal }) => {
  const dispatch = useDispatch()
  const queryClient = useQueryClient();
  
  const { mutate: ClearCart } = useAuthMutation(clearUserCart(), {
		onSuccess: (data) => {
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-[#00000080] bg-opacity-30">
      <div id="modal" className="bg-white-100 p-4 rounded-lg w-[90%] md:w-[50%] lg:w-[24%] h-415 text-center ">
        ...
        <div>
          ...
          {/* Modal content */}
          <h2 className="text-[18px] font-bold pt-2 text-[#101828]">Clear Cart</h2>
